...
namespace {
	...
	function fastcloudwp_vite_enqueue( $entry, array $deps = array() ): void {
		...
		$plugin_data = get_plugin_data( FASTCLOUDWP_PLUGIN_DIR . 'fastcloud-offload-media.php' );
	...
	function fastcloudwp_load_js_translations(): array {
		...
		$json_files = glob( $lang_dir . 'fastcloud-offload-media-' . $locale . '-*.json' );
	...
	function fastcloudwp_enqueue_app_state(): void {
		...
		if ( ! isset( $_GET['page'] ) || 'fastcloud-offload-media' !== $_GET['page'] ) {
			return;
		}
