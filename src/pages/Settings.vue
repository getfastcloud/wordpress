<template>
  <div class="tw:max-w-3xl">
    <div v-if="fastcloudwp.state.account_status === 'pending_register'" class="tw:border tw:bg-white tw:border-neutral-200 tw:shadow tw:rounded-md tw:mb-6 tw:p-6">
      <p class="tw:font-semibold tw:text-[16px] tw:mb-1">{{ __('Confirm your account', 'fastcloud-offload-media') }}</p>
      <p class="tw:text-neutral-500 tw:mb-4">{{ __('We sent a confirmation link to your email. Click it to create your account. If you used the wrong address, update it below and resend.', 'fastcloud-offload-media') }}</p>
      <div>
        <label class="tw:font-semibold tw:text-sm tw:block tw:mb-1" for="resend-email">{{ __('Email address', 'fastcloud-offload-media') }}</label>
        <div class="tw:flex tw:gap-2 tw:items-center tw:w-full">
          <Input v-model="resendEmail" type="email" id="resend-email" :error="resendError" placeholder="you@example.com" class="tw:grow"/>
          <Button variant="outline" :loading="resending" @click.prevent="resend">
            {{ resending ? __('Sending…', 'fastcloud-offload-media') : __('Resend confirmation email', 'fastcloud-offload-media') }}
          </Button>
        </div>
        <p v-if="resendSuccess" class="tw:m-0 tw:mt-2 tw:text-sm tw:text-green-700">{{ __('Email sent. Check your inbox.', 'fastcloud-offload-media') }}</p>
      </div>
    </div>

    <div v-if="fastcloudwp.state.account_status === 'pending_approve'" class="tw:border tw:bg-white tw:border-neutral-200 tw:shadow tw:rounded-md tw:mb-6 tw:p-6">
      <p class="tw:font-semibold tw:text-[16px] tw:mb-1">{{ __('Approve this site connection', 'fastcloud-offload-media') }}</p>
      <p class="tw:text-neutral-500 tw:mb-4">{{ __('We sent an email to your FastCloudWP account asking you to approve adding this site. Check your inbox and click the link.', 'fastcloud-offload-media') }}</p>
      <div class="tw:flex tw:flex-col tw:gap-2 tw:max-w-xs">
        <Button variant="outline" :loading="resendingApprove" @click.prevent="resendApprove">
          {{ resendingApprove ? __('Sending…', 'fastcloud-offload-media') : __('Resend approval email', 'fastcloud-offload-media') }}
        </Button>
        <p v-if="resendApproveSuccess" class="tw:m-0 tw:text-sm tw:text-green-700">{{ __('Email sent. Check your inbox.', 'fastcloud-offload-media') }}</p>
        <p v-if="resendApproveError" class="tw:m-0 tw:text-sm tw:text-red-600">{{ resendApproveError }}</p>
      </div>
    </div>
    ...
    <div class="tw:border tw:bg-white tw:border-neutral-200 tw:shadow tw:rounded-md tw:mb-6 tw:p-6">
      ...
      <Notice v-if="fastcloudwp.state.account_status !== 'active'" variant="info" class="tw:mb-3">{{ __('Your storage is currently limited to 1 GB. Confirm your account to unlock your full storage quota.', 'fastcloud-offload-media') }}</Notice>
...
const resendEmail = ref(fastcloudwp.value.state.account_email)
const resending = ref(false)
const resendSuccess = ref(false)
const resendError = ref<string | undefined>()
...
const resend = async () => {
  ...
  try {
    const {response, data} = await apiFetch<{success: boolean; error?: string}>(
      '/wp-json/fastcloudwp/v1/resend-confirmation',
      {
        method: 'POST',
    ...
    if (!response.ok) {
      resendError.value = data.error
      return
    }

    fastcloudwp.value.state.account_email = resendEmail.value
...
const resendingApprove = ref(false)
const resendApproveSuccess = ref(false)
const resendApproveError = ref<string | undefined>()

const resendApprove = async () => {
  resendingApprove.value = true
  resendApproveSuccess.value = false
  resendApproveError.value = undefined
  ...
  try {
    ...
    if (!response.ok) {
      resendApproveError.value = data.error
      return
    }

    resendApproveSuccess.value = true
  } finally {
    resendingApprove.value = false
  }
}
