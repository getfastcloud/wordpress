...
class Rest {
	...
	public function quick_start( \WP_REST_Request $request ): array|\WP_REST_Response {
		...
		if ( 409 === $code ) {
			return new \WP_REST_Response( array( 'error' => __( 'This website is already connected to an account.', 'fastcloud-offload-media' ) ), 409 );
		}

		if ( 422 === $code ) {
			return new \WP_REST_Response( array( 'error' => __( 'Domain verification failed. Make sure the site is publicly reachable.', 'fastcloud-offload-media' ) ), 422 );
		}
		...
		do_action( 'fastcloudwp_connected', $body, '' );
	...
	public function connect( \WP_REST_Request $request ): array|\WP_REST_Response {
		...
		if ( $response->failed() ) {
			return new \WP_REST_Response( array( 'error' => __( 'Invalid Site Key', 'fastcloud-offload-media' ) ), $code );
		}
