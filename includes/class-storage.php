...
class Storage {
	...
	/**
	 * Sync quota from a webhook body payload (not HTTP headers).
	 *
	 * @param int  $total    Total quota in bytes.
	 * @param int  $used     Used bytes.
	 * @param bool $exceeded Whether the quota is exceeded.
	 */
	public function sync_from_webhook( int $total, int $used, bool $exceeded ): bool {
		return $this->update(
			array(
				'total'     => $total,
				'used'      => $used,
