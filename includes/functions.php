...
namespace {
	...
	function fastcloudwp_delete_files_for_attachment( int $attachment_id ): bool {
		...
		$files = array_map( 'fastcloudwp_resolve_path', $files );
	...
	function fastcloudwp_javascript_state(): array {
		...
		return array(
			...
			'state'      => array(
				...
				'account_status' => get_option( 'fastcloudwp_account_status', 'active' ),
				'account_email'  => (string) get_option( 'fastcloudwp_account_email', '' ),
				...
				'cdn_ready'      => fastcloudwp_settings()->cdn_ready(),
	...
	/**
	 * Returns the real path to a file, trying Unicode normalization variants when the original path is not found.
	 *
	 * Linux stores filenames as raw bytes; a path from WordPress (NFC) may not match a filename
	 * written in NFD form (common after macOS migrations). Tries NFC then NFD before giving up.
	 *
	 * @param string $path The path to resolve.
	 */
	function fastcloudwp_resolve_path( string $path ): string {
		if ( file_exists( $path ) || ! function_exists( 'normalizer_normalize' ) ) {
			return $path;
		}
		foreach ( array( \Normalizer::NFC, \Normalizer::NFD ) as $form ) {
			$normalized = normalizer_normalize( $path, $form );
			if ( $normalized && $normalized !== $path && file_exists( $normalized ) ) {
				return $normalized;
			}
		}
	...
	function fastcloudwp_remove_site_options(): void {
		...
		delete_option( 'fastcloudwp_cdn_ready' );
		delete_option( 'fastcloudwp_account_status' );
		delete_option( 'fastcloudwp_account_email' );
	}
