...
class Offloader {
	...
	public function offload_attachments( array $attachment_ids ): array {
		/**
		 * Filters the list of attachment IDs before offloading begins.
		 *
		 * Return a modified array to skip or add attachments. Return an empty
		 * array to abort the entire batch.
		 *
		 * @param int[] $attachment_ids Attachment post IDs about to be offloaded.
		 */
		$attachment_ids = (array) apply_filters( 'fastcloudwp_before_offload', $attachment_ids );
		...
		if ( $response['success'] && ! empty( $response['api_response'] ) ) {
			...
			foreach ( $response['api_response']['offloaded'] ?? array() as $id ) {
				...
				/**
				 * Fires when an attachment has been successfully offloaded to FastCloud.
				 *
				 * @param int $id The attachment post ID.
				 */
				do_action( 'fastcloudwp_attachment_offloaded', $id );
	...
	public function build_file_payload( int $attachment_id ): array {
		...
		$path     = fastcloudwp_resolve_path( get_attached_file( $attachment_id ) );
		...
		if ( ! empty( $meta['original_image'] ) ) {
			...
			$orig_path = fastcloudwp_resolve_path( $base_path . $orig );
			$orig_url  = $base_url . basename( $orig_path );

			if ( file_exists( $orig_path ) ) {
				$files[] = array(
					'original_url'      => $orig_url,
					'original_filename' => basename( $orig_path ),
					'type'              => 'original',
					'mime'              => $mime,
		...
		if ( ! empty( $meta['sizes'] ) && is_array( $meta['sizes'] ) ) {
			foreach ( $meta['sizes'] as $variant ) {
				...
				$file         = $variant['file'];
				$variant_path = fastcloudwp_resolve_path( $base_path . $file );
				$variant_url  = $base_url . basename( $variant_path );

				if ( file_exists( $variant_path ) ) {
					$files[] = array(
						'original_url'      => $variant_url,
						'original_filename' => basename( $variant_path ),
						'type'              => 'variant',
						'mime'              => $mime,
